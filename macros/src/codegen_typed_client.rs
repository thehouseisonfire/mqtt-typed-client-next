...
impl<'a> TypedClientGenerator<'a> {
    ...
    fn generate_typed_client_impl(&self) -> proc_macro2::TokenStream {
        ...
        let publisher_methods = if self.code_generator.should_generate_publisher() {
            self.generate_all_publisher_methods()
        } else {
            quote! {}
        };
        let subscribe_method = if self.code_generator.should_generate_subscriber() {
...
mod tests {
    ...
    fn create_test_code_generator() -> crate::codegen::CodeGenerator {
        let context =
            StructAnalysisContext::from_components(Some(syn::parse_quote! { f64 }), false, vec![]);
