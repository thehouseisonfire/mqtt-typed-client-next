...
impl CodeGenerator {
    ...
    fn generate_builder_methods(&self) -> proc_macro2::TokenStream {
        ...
        let (serializer_ty, client_expr, extra_bounds) =
            if let Some(s) = &self.macro_args.custom_serializer {
                (
                    quote! { #s },
                    quote! { client.clone_with_serializer::<#s>() },
                    quote! { , #s: Default + Clone + Send + Sync + 'static },
                )
            } else {
    ...
    fn generate_subscriber_methods(&self) -> proc_macro2::TokenStream {
        ...
        let (serializer_ty, where_clause) = if let Some(s) = &self.macro_args.custom_serializer {
            (
                quote! { #s },
                quote! {
                    where
    ...
    fn generate_publisher_methods(&self) -> Result<proc_macro2::TokenStream, syn::Error> {
        ...
        let (serializer_ty, client_expr, where_clause) = if let Some(s) =
            &self.macro_args.custom_serializer
        {
            (
                quote! { #s },
                quote! { client.clone_with_serializer::<#s>() },
    ...
    fn generate_for_methods(&self) -> Vec<(proc_macro2::TokenStream, proc_macro2::TokenStream)> {
        self.context
            .topic_params
            .iter()
    ...
    fn generate_single_for_method(
        ...
        let method_name = format_ident!("for_{}", param.get_publisher_param_name());
    ...
    pub fn get_topic_format_and_args(&self) -> (String, Vec<proc_macro2::TokenStream>) {
