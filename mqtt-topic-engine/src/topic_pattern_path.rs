...
impl TopicPatternPath {
	...
	pub fn resolve_bound_segments(&self) -> Vec<TopicPatternItem> {
		self.parameter_bindings.as_ref().map_or_else(
			|| self.segments.clone(),
			|bindings| self.apply_bindings_to_segments(bindings),
		)
	}
	...
	pub fn with_cache_strategy(&self, new_cache: CacheStrategy) -> Self {
		...
		new_pattern
			.parameter_bindings
			.clone_from(&self.parameter_bindings);
		new_pattern
	}
				...
				| Some(cache_mutex) => {
					{
						let mut match_cache = cache_mutex
							.lock()
							.map_err(|_| TopicMatchError::CachePoisoned)?;
					...
					let topic_match =
						self.try_match_internal(Arc::clone(&topic))?;
...
impl<'a> IntoIterator for &'a TopicPatternPath {
	type Item = &'a TopicPatternItem;
	type IntoIter = Iter<'a, TopicPatternItem>;
