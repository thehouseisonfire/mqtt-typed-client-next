...
impl CacheStrategy {
	...
	///
	/// When the `lru-cache` feature is disabled, all capacities resolve to
	/// [`Self::NoCache`]. A non-zero capacity emits a warning so callers can
	/// notice the disabled feature.
	#[must_use]
