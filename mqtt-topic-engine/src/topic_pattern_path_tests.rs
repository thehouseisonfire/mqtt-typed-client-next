...
trait TopicPatternPathTestExt {
    fn with_parameters<I, K, V>(self, params: I) -> Result<Self, TopicPatternError>
    where
        Self: Sized,
...
impl TopicPatternPathTestExt for TopicPatternPath {
    fn with_parameters<I, K, V>(mut self, params: I) -> Result<Self, TopicPatternError>
    where
        I: IntoIterator<Item = (K, V)>,
        K: Into<ArcStr>,
        V: Into<ArcStr>,
...
mod with_parameters_tests {
    ...
    fn test_multiple_parameter_substitution() {
        let pattern = create_pattern("buildings/{building}/floors/{floor}/rooms/{room}");
    ...
    fn test_partial_parameter_substitution() {
        let pattern = create_pattern("sensors/{sensor_id}/data/{measurement_type}");
        let result = pattern
    ...
    fn test_with_hashmap() {
        let pattern = create_pattern("devices/{device_type}/{device_id}/status");
    ...
    fn test_parameter_not_found_error() {
        ...
        if let Err(TopicPatternError::WildcardUsage { usage }) = result {
            assert!(usage.contains("Parameter 'nonexistent' not found in pattern"));
        } else {
    ...
    fn test_mixed_valid_and_invalid_parameters() {
        ...
        if let Err(TopicPatternError::WildcardUsage { usage }) = result {
            assert!(usage.contains("Parameter 'invalid_param' not found in pattern"));
        }
    }
    ...
    fn test_mixed_named_and_anonymous_wildcards() {
        let pattern = create_pattern("devices/{device_type}/+/status/{status_type}");
    ...
    fn test_special_characters_in_values() {
        ...
        assert_eq!(result.mqtt_pattern(), "devices/temp-sensor@home.local/data");
    }
    ...
    fn str_to_topic_pattern_path(topic: &str) -> Result<TopicPatternPath, TopicPatternError> {
    ...
    fn test_special_characters() {
        let result = str_to_topic_pattern_path("device-123/status@home").unwrap();
        assert_eq!(
            result.segments(),
            &vec![
...
mod integration_tests {
    ...
    fn test_iot_device_telemetry_pattern() {
        ...
        let result = pattern.with_parameters(params).unwrap();
        let expected = "iot/office-a/3/devices/temperature/temp-001/telemetry/celsius";
    ...
    fn test_selective_filtering() {
        ...
        let pattern = create_pattern("sensors/{location}/{sensor_type}/{sensor_id}/data");
    ...
    fn test_log_aggregation_pattern() {
        ...
        // topic_pattern() preserves original template
        assert_eq!(result.topic_pattern(), "logs/{service}/{level}/{details:#}");
